...
Needs["AstronomicalDiaries`Modeling`Updates`"]
...
fitModel[observations_, steps_, vars_ : {}] :=
	...
	timeCatDistPrior, deltaParams, deltaStar, outliers, inliers},
		...
		outliers = Position[m, 1, {1}, Heads -> False][[All, 1]];
		inliers = Position[m, 0, {1}, Heads -> False][[All, 1]];

		muTimesPrior = AssociationMap[NormalDistribution[0, 6] &, Union@timeCats];
		muTimes = RandomVariate /@ muTimesPrior;
		sigma2TimesPrior = AssociationMap[InverseGammaDistribution[1/2, 1/2] &, Union@timeCats]; sigma2Times = RandomVariate /@ sigma2TimesPrior;
		t = normalArraySample@NormalDistribution[Lookup[muTimes, timeCats], Sqrt@Lookup[sigma2Times, timeCats]];
			...
			Function[
				...
				Scan[
					With[{obsT = Pick[t, timeCats, #]},
						muTimes[#] = normalNormalMeanSample[muTimesPrior[#], Sqrt[sigma2Times[#]], obsT];
						sigma2Times[#] = normalNormalVarianceSample[sigma2TimesPrior[#], muTimes[#], obsT]
					]&,
					Keys@muTimes
				];
